...
class ProductCheckout extends Component
{
    ...
    public function checkout(PaystackService $paystack)
    {
        ...
        $pricing = $this->selectedPricingId ? ProductPricing::find($this->selectedPricingId) : null;
        ...
        OrderAddon::where('user_product_id', $userProduct->id)->delete();
        foreach ($selectedAddons as $addon) {
            OrderAddon::create([
    ...
    public function render()
    {
        ...
        if ($product) {
            if ($this->supportsCloud) {
                $cloudData = $this->loadPricingData($product, 'cloud');
            }
            if ($this->supportsOnPrem) {
                $onpremData = $this->loadPricingData($product, 'onprem');
            }
        }
